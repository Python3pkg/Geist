...
def normalise_correlation(image_tile_dict, transformed_array, template, normed_tolerance=1):
    ...
    image_norms = {(x,y):np.linalg.norm(image_tile_dict[(x,y)])*template_norm for (x,y) in list(image_tile_dict.keys())}
    match_points = list(image_tile_dict.keys())
    ...
    result = {key:value for key, value in list(image_matches_normalised.items()) if np.round(value, decimals=3) >= normed_tolerance}
...
def normalise_correlation_coefficient(image_tile_dict, transformed_array, template, normed_tolerance=1):
    ...
    image_norms = {(x,y):np.linalg.norm(image_tile_dict[(x,y)]- np.mean(image_tile_dict[(x,y)]))*template_norm for (x,y) in list(image_tile_dict.keys())}
    match_points = list(image_tile_dict.keys())
    # for correlation, then need to transofrm back to get correct value for division
    h, w = template.shape
    image_matches_normalised = {match_points[i]:transformed_array[match_points[i][0], match_points[i][1]]/image_norms[match_points[i]] for i in range(len(match_points))}
    normalised_matches = {key:value for key, value in list(image_matches_normalised.items()) if np.round(value, decimals=3) >= normed_tolerance}
    return list(normalised_matches.keys())
...
def calculate_squared_differences(image_tile_dict, transformed_array, template, sq_diff_tolerance=0.1):
    ...
    image_norms_squared = {(x,y):np.sum(image_tile_dict[(x,y)]**2) for (x,y) in list(image_tile_dict.keys())}
    match_points = list(image_tile_dict.keys())
    # for correlation, then need to transofrm back to get correct value for division
    h, w = template.shape
    ...
    normalised_matches = {key:value for key, value in list(image_matches_normalised.items()) if np.round(value, decimals=3) <= cutoff}
    return list(normalised_matches.keys())
